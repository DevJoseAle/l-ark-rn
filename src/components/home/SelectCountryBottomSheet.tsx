...
import { COUNTRIES, CountryCode, getCountryFlag, getCountryName } from "@/src/types/campaign-create.types";
      ...
      >
        <BottomSheetView style={styles.content}>
          <FlatList
            ...
            renderItem={
              ({ item }) => <BottomListItem country={item.code}
                setCty={setSelectedCountry} isSelected={selectedCountry === item.code} />
            }
            keyExtractor={(item) => item.code}
            ListFooterComponent={<SelectButton disabled={disabledBtn} setCountry={handleSubmit} country={selectedCountry} ref={ref} />}
...
const ListHeaderComponent = () => {
  return (
    <>
      ...
      <Text style={{ fontSize: 15, fontWeight: 'light', fontStyle: 'italic' ,marginVertical: 3, marginLeft: 15, color: Colors.light.secondaryText }}
      >
...
const BottomListItem = ({ country, setCty, isSelected }: ListItemProps) => {
  ...
  return (
    ...
    activeOpacity={0.6}
      ...
      <Text style={{ fontSize: 18, marginLeft: 10, color: isSelected ? 'white' : 'black' }}>{name} ({code})</Text>
    </TouchableOpacity>
  )
};

const SelectButton = ({ disabled, setCountry, country, ref }: { disabled: boolean, setCountry: (country: CountryCode) => void, country: any, ref: React.ForwardedRef<BottomSheetModal> }) => {
  return (
    <TouchableOpacity style={{
