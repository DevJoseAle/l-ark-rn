...
export const BeneficiariesSection = () => {
  ...
  useEffect(() => {
    const searchUsers = async () => {
      ...
      setIsSearching(true);
      try {
        const results = await UserSearchService.searchUsers(debouncedSearch, currentUserId);
  ...
  const handleAddBeneficiary = (user: BeneficiaryUser) => {
    const isAlreadyAdded = formData.beneficiaries.some((b) => b.user.id === user.id);
    ...
    const newShareValue = currentCount === 0 ? 100 : Math.floor(100 / (currentCount + 1));
  ...
  const totalPercentage = formData.beneficiaries.reduce((sum, b) => sum + b.shareValue, 0);
      ...
      >
        ...
        <View style={styles.header}>
          <View style={styles.titleContainer}>
            <Text style={[styles.sectionTitle, { color: colors.text }]}>Beneficiarios</Text>
        ...
        <View style={styles.countrySection}>
          ...
          {/* Error de país */}
          {countryError && (
            <Text style={[styles.errorText, { color: colors.error }]}>{countryError.message}</Text>
          )}
        </View>
